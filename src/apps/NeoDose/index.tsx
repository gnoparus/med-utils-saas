...
import { Zap, Droplets, Pill, Heart, AlertTriangle, ChevronRight, Lock } from 'lucide-react'

// ─── Broselow Band Data ───────────────────────────────────────────────────────
const BROSELOW_BANDS = [
  { name: 'Grey',   label: 'GREY',   min: 3,  max: 5,  color: '#9ca3af', bg: 'rgba(156,163,175,0.15)', border: 'rgba(156,163,175,0.4)' },
  { name: 'Pink',   label: 'PINK',   min: 5,  max: 7,  color: '#f472b6', bg: 'rgba(244,114,182,0.15)', border: 'rgba(244,114,182,0.4)' },
  { name: 'Red',    label: 'RED',    min: 7,  max: 9,  color: '#ef4444', bg: 'rgba(239,68,68,0.15)',   border: 'rgba(239,68,68,0.4)'   },
  { name: 'Purple', label: 'PURPLE', min: 9,  max: 11, color: '#a855f7', bg: 'rgba(168,85,247,0.15)', border: 'rgba(168,85,247,0.4)' },
  { name: 'Yellow', label: 'YELLOW', min: 11, max: 14, color: '#eab308', bg: 'rgba(234,179,8,0.15)',   border: 'rgba(234,179,8,0.4)'   },
  { name: 'White',  label: 'WHITE',  min: 14, max: 18, color: '#e2e8f0', bg: 'rgba(226,232,240,0.15)',border: 'rgba(226,232,240,0.4)' },
  { name: 'Blue',   label: 'BLUE',   min: 18, max: 25, color: '#3b82f6', bg: 'rgba(59,130,246,0.15)', border: 'rgba(59,130,246,0.4)'  },
  { name: 'Orange', label: 'ORANGE', min: 25, max: 36, color: '#f97316', bg: 'rgba(249,115,22,0.15)', border: 'rgba(249,115,22,0.4)'  },
  { name: 'Green',  label: 'GREEN',  min: 36, max: 50, color: '#22c55e', bg: 'rgba(34,197,94,0.15)',  border: 'rgba(34,197,94,0.4)'   },
]

function getBand(kg: number) {
  return (
...
const WEIGHT_PRESETS = [3, 5, 7, 10, 12, 15, 20, 25, 30]
    ...
    >
      ...
      {med.locked && (
        <div className="absolute inset-0 rounded-3xl z-20 backdrop-blur-sm bg-slate-950/60 flex flex-col items-center justify-center gap-2">
          ...
          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Pro Only</span>
        </div>
      )}
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="font-bold text-slate-100 text-sm leading-tight">{med.name}</div>
              <div className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">{med.sub}</div>
            </div>
          </div>
          <span
        ...
        <div className="flex items-end justify-between">
          <div className="flex items-baseline gap-1.5">
            ...
            <span className="text-sm font-bold text-slate-400 mb-0.5">{med.unit}</span>
          </div>

          {med.volume !== null && (
            <div
              className="text-right px-3 py-2 rounded-2xl"
              style={{ background: `${med.accent}10`, border: `1px solid ${med.accent}20` }}
            >
              <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-0.5">Volume</div>
              <div className="flex items-baseline gap-0.5">
                <span className="text-xl font-black" style={{ color: med.accent }}>
                  {med.volume.toFixed(1)}
                </span>
                <span className="text-xs text-slate-400 font-semibold">{med.volUnit}</span>
              </div>
              {med.concentration && (
                <div className="text-[9px] text-slate-600 font-mono mt-0.5">{med.concentration}</div>
              )}
            </div>
          )}
        </div>
      ...
      <div
        ...
        style={{ background: `linear-gradient(to right, transparent, ${med.accent}, transparent)` }}
      />
    </motion.div>
  )
}
      ...
      {BROSELOW_BANDS.map(band => {
        ...
        return (
          <motion.div
            ...
            animate={{ opacity: isActive ? 1 : 0.2, scaleY: isActive ? 1.4 : 1 }}
...
function NumpadWeight({ value, onChange }: { value: number; onChange: (v: number) => void }) {
      ...
      <div className="grid grid-cols-3 gap-2">
        {keys.map(k => (
          <motion.button
            key={k}
            whileTap={{ scale: 0.88 }}
            onPointerDown={() => { setActive(true); handleDigit(k) }}
    ...
    >
      ...
      >
        ...
        <motion.div
          ...
          animate={{ background: `radial-gradient(ellipse 70% 90% at 50% 0%, ${band.color}22 0%, transparent 80%)` }}
        ...
        <div className="flex items-center justify-between mb-5 relative z-10">
          ...
          <div className="text-center">
            <h1 className="font-black text-lg tracking-tight text-white">NeoDose</h1>
            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">Peds Resuscitation</p>
          </div>
          <div className="w-20" />
        </div>
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Est. Weight</span>
            ...
            <motion.button
              ...
              style={{ background: `${band.color}18`, border: `1px solid ${band.color}40` }}
            >
              <motion.span
              ...
              <ChevronRight
                ...
                style={{ transform: showNumpad ? 'rotate(90deg)' : 'rotate(0deg)' }}
              />
            </motion.button>
          </div>
          ...
          <div className="flex gap-1.5 mb-3 overflow-x-auto pb-1 scrollbar-none">
            {WEIGHT_PRESETS.map(w => (
              <motion.button
                key={w}
                whileTap={{ scale: 0.88 }}
                onClick={() => { setWeight(w); if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate(10) }}
                className="shrink-0 px-3 py-1.5 rounded-full text-xs font-bold transition-all"
                style={
                  weight === w
                    ? { background: band.color, color: '#000', border: `1px solid ${band.color}` }
                    : { background: 'rgba(255,255,255,0.05)', color: 'rgb(148,163,184)', border: '1px solid rgba(255,255,255,0.08)' }
                }
              >
                {w} kg
              </motion.button>
          ...
          <div className="mt-3">
            ...
            <div className="flex justify-between items-center mt-2">
              ...
              <span className="text-[10px] text-slate-600 font-semibold">Broselow Tape</span>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <AnimatePresence>
        {showNumpad && (
          ...
          >
            <div className="p-4">
              <NumpadWeight value={weight} onChange={v => { setWeight(v); }} />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 space-y-3 pb-24">
        ...
        <div className="flex items-center gap-3 mb-1">
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Code Blue Essentials</span>
          <div className="flex-1 h-px bg-slate-800" />
          <span className="text-[10px] text-emerald-500 font-bold uppercase tracking-widest bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full">Free</span>
        </div>
        ...
        <div className="flex items-center gap-3 mt-5 mb-1">
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Advanced Pharmacy</span>
          <div className="flex-1 h-px bg-slate-800" />
          <span className="text-[10px] text-amber-400 font-bold uppercase tracking-widest bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded-full">Pro</span>
        </div>
